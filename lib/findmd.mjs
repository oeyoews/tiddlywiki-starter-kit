// prompt: 写一个mjs脚本, 找到所有的.md和.meta扩展文件并且放在新的目录下面, 使用mjs语法, 新的目录名字可以指定, 将meta文件放在目录文件的meta目录, 输出当前操作系统,md meta数量等信息
...
function findAndMoveMdFiles(sourceDir, destinationDir) {
  ...
  console.log(`Number of .md files moved: ${mdCount}`);
  console.log(`Number of .meta files moved: ${metaCount}`);
}
